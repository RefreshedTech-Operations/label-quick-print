...
export default function Orders() {
  ...
  const [filter, setFilter] = useState<'all' | 'printed' | 'unprinted' | 'exceptions' | 'bundled' | 'non_bundled' | 'incomplete_bundles'>(() => {
    const urlFilter = searchParams.get('filter');
    if (urlFilter && ['all', 'printed', 'unprinted', 'exceptions', 'bundled', 'non_bundled', 'incomplete_bundles'].includes(urlFilter)) {
      return urlFilter as 'all' | 'printed' | 'unprinted' | 'exceptions' | 'bundled' | 'non_bundled' | 'incomplete_bundles';
    }
    return 'unprinted';
  });
      ...
      <div className="flex gap-4 items-center flex-wrap">
        <Select value={filter} onValueChange={(v: any) => setFilter(v)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="non_bundled">Non-Bundled Items</SelectItem>
