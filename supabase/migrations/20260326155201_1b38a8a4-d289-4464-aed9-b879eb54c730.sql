...
-- Drop existing function first (has 8 params)
...
-- Also update the stats function (6-param version with p_channel)
DROP FUNCTION IF EXISTS public.get_shipments_stats_with_archive(text, date, boolean, text, boolean, text);
...
BEGIN
  ...
  WITH combined AS (
    SELECT s.printed, s.manifest_url, s.cancelled, s.show_date, s.bundle, s.order_group_id,
           ...
           s.location_id, s.address_full, s.channel, s.unit_id, false as is_archived
    FROM shipments s
    ...
    SELECT sa.printed, sa.manifest_url, sa.cancelled, sa.show_date, sa.bundle, sa.order_group_id,
           ...
           sa.location_id, sa.address_full, sa.channel, sa.unit_id, true as is_archived
    FROM shipments_archive sa
    WHERE p_include_archive = true
  )
  SELECT 
  ...
  WHERE 
    ...
    AND (
      ...
      OR search_term = ''
      OR (p_strict = true AND (
        UPPER(TRIM(COALESCE(c.uid, ''))) = UPPER(TRIM(search_term))
        OR UPPER(TRIM(COALESCE(c.order_id, ''))) = UPPER(TRIM(search_term))
        OR UPPER(TRIM(COALESCE(c.buyer, ''))) = UPPER(TRIM(search_term))
        OR UPPER(TRIM(COALESCE(c.tracking, ''))) = UPPER(TRIM(search_term))
        OR UPPER(TRIM(COALESCE(c.unit_id, ''))) = UPPER(TRIM(search_term))
        OR UPPER(TRIM(COALESCE(c.location_id, ''))) = UPPER(TRIM(search_term))
      ))
      OR (p_strict = false AND (
