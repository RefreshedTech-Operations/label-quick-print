-- Harden function: lock search_path to avoid SECURITY DEFINER search_path hijacking
CREATE OR REPLACE FUNCTION public.archive_old_shipments(
  days_to_keep integer DEFAULT 10,
...
BEGIN
  IF days_to_keep IS NULL OR days_to_keep < 1 THEN
    RAISE EXCEPTION 'days_to_keep must be an integer >= 1';
  END IF;

  IF batch_size IS NULL OR batch_size < 1 THEN
    RAISE EXCEPTION 'batch_size must be an integer >= 1';
  END IF;
  ...
  SELECT COUNT(*) INTO v_remaining_count FROM public.shipments;
