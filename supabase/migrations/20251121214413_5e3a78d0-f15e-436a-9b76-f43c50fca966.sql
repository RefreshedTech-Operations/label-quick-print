...
-- Drop all existing versions of get_daily_analytics to resolve function overloading conflict
DROP FUNCTION IF EXISTS public.get_daily_analytics(date, date);
DROP FUNCTION IF EXISTS public.get_daily_analytics(timestamp with time zone, timestamp with time zone);

-- Recreate the correct version with proper LEFT JOIN
