...
-- Drop both overloads and recreate with channel in return

DROP FUNCTION IF EXISTS public.search_all_shipments(text, date, boolean, text, integer, integer, boolean);
DROP FUNCTION IF EXISTS public.search_all_shipments(text, date, boolean, text, integer, integer, boolean, text);

-- Recreate 8-param version (with p_channel) adding channel to return
...
RETURNS TABLE(
  ...
  is_archived boolean, channel text
)
